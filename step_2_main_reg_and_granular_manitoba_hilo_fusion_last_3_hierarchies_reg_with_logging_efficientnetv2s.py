...
# from utilities.efficientNet_FPN.efficientnet_hilo_fusion_encoder_with_classification_head import model_Hilo
from utilities.efficientNet_FPN.efficientnetv2s_hilo_fusion_encoder_with_reg_head_last_3_hierarchies import model_Hilo
...
opt['model_path'] = './output/Step_1_DE_Ev2s_DRSAT_EFFM'
...
opt['weight_decay'] = .0
opt['loss_option'] = 'MSE'
opt['model_name'] = 'Hybrid_TFN_CT_DE'
opt['patience'] = 40
...
if __name__ ==  '__main__':
    ...
    val_dataset = manitoba_1916_npy_cropped_reg_and_gran(data_path=opt['data_path'], file_path=opt['file_path'], transforms=val_transforms)
    ...
    for curr_fold, (train_split, valid_split) in enumerate(splits):  
        ...
        ################### Model ###################
        model = model_Hilo().to(opt['device'])
        ...
        torch.save(trained_model.state_dict(), "%s_%s_%.4facc_%dth-fold_lr-%.5f_sen-%.2f_spec-%.3f_corr-%.3f_epoch-%.2f_pv-%.2f_rho-%.2f_prho-%.2f-bill-all-de.pt"%(opt['model_name'],opt['loss_option'], data['best_acc'], 
